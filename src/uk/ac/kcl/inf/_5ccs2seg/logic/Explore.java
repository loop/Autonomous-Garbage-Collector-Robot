			...
			public void run() {
				...
				while (!getFlag()) {
					...
					for (int i = 0; i < 9; i++) {
						...
						alpha = Math.toRadians(Bot.turnBy(cleaner1.getHead(),
								ang));
						range = cleaner1.getRange(i);
						x = cleaner1.getX();
						y = cleaner1.getY();
						arr = calcCoord(x, y, alpha, range);
						// if (i==4){System.out.println(arr[0] + "---" +
						// arr[1]); }
						ang = ang + 5;
					...
					if (cleaner1.getFidCount() > 0) {
						...
						for (int i = 0; i < fiduc.length; i++) {
							...
							for (int j = 0; j < size; j++) {
								d = Math.sqrt(Math.pow((arr[0] - garbageL
										.get(j)[0]), 2)
										+ Math.pow(
												(arr[1] - garbageL.get(j)[1]),
												2));
				...
				while (!getFlag()) {
					...
					if (path != null) {
						for (int i = 0; i < path.size(); i++) {
							map.setSts(path.get(i).getArr(1), path.get(i)
									.getArr(0), 1);
						}
					}
					...
					for (int i = path.size() - 1; i >= 0; i--) {
						map.setSts(path.get(i).getArr(1),
								path.get(i).getArr(0), 4);
						arrr = map.arrayIndexToCoordCalc(path.get(i).getArr(1),
								path.get(i).getArr(0));
						road.add(arrr);
					}
