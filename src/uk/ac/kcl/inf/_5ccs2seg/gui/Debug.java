...
public class Debug {
	...
	public Debug(final Bot bot, int botNo) {
        ...
        */
		...
		Thread reportThread = new Thread() {
			public void run() {
				while (true) {
					positionLabel.setText(String.format("(%4.2f,%4.2f)", bot
							.getX(), bot.getY()));
					yawLabel.setText(String.format("%4.4f rad, %3.0f deg", bot
							.getYaw(), bot.getYaw() * 180 / Math.PI));
					...
					if (bot.getGripSts() == 1) {
						sts = "Open";
					} else if (bot.getGripSts() == 2) {
						sts = "Closed";
					} else {
						sts = "In transition";
					}

					if (bot.getGripBeam() != 0) {
						beam = "Yes";
					} else {
						beam = "No";
					}
	...
	private void calcFid(PlayerFiducialItem fid) {
		...
		fid2 = String.format("%4.2f", Math.sqrt(Math.pow(fid.getPose().getPy(),
				2)
				+ Math.pow(fid.getPose().getPx(), 2)));
	}
